  ...
  if (!selectedNode) {
    return (
      <section className="panel flex min-h-[620px] flex-col items-center justify-center p-8 text-center">
        ...
        <h2 className="text-lg font-black text-slate-100">Select an asset</h2>
        <p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">
          Choose a node in the resource tree to edit art direction, generation parameters, prompt overrides, and sprite preview settings.
        </p>
      </section>
    );
  }
    ...
    <section className="panel sticky top-5 min-h-[620px] p-4">
      ...
      <div className="thin-scrollbar max-h-[calc(100vh-9rem)] space-y-4 overflow-y-auto pr-1">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="field-label">Animation Frames</label>
            <input
              type="number"
              className="field-input p-2 text-sm"
        ...
        <div>
          <label className="field-label">Node Style Override</label>
          <select
            className="field-input p-2 text-sm"
        ...
        <div className="grid grid-cols-[1fr_auto] gap-3">
          <div>
            <label className="field-label">Node Seed</label>
            <input
              type="number"
              className="field-input p-2 text-sm"
        ...
        <div>
          <div className="mb-2 flex items-center justify-between gap-2">
            <label className="field-label mb-0">Prompt</label>
        ...
        <div className="rounded-lg border border-slate-700 bg-slate-950/55 p-3">
          ...
          {selectedNode.resultUrl ? (
            ...
            <div className="flex aspect-square w-full flex-col items-center justify-center rounded-md border border-dashed border-slate-700 bg-slate-900/60 p-6 text-center">
              ...
              <p className="text-sm font-bold text-slate-300">No render yet</p>
              <p className="mt-1 text-xs leading-5 text-slate-500">Save and generate assets to preview this node here.</p>
            </div>
          )}
        </div>
